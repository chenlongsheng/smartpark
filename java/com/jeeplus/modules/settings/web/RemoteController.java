...
public class RemoteController {
	...
	public String remoteControl(String channelId, String opr) throws TimeoutException {
		...
		try {
			...
			String message = "{\"mac\":\"" + tDeviceDetail.getMac() + "\",\"bus_addr\":\"" + tDeviceDetail.getBusAddr()
					+ "\",\"channels\":{\"" + tChannel.getChNo() + "\":{\"value\":\"" + opr + "\"}}}";
			System.out.println(message);


			// 发送消息到队列中
			rmqChannel.basicPublish("org.10",
					"dev.command." + tDeviceDetail.getMac().substring(0, tDeviceDetail.getMac().indexOf("_")), null,
					message.getBytes("UTF-8"));
			logger.debug("/**============================================================**/");
			logger.debug("Producer Send +'" + message + "'  to " + "dev.command."
					+ tDeviceDetail.getMac().substring(0, tDeviceDetail.getMac().indexOf("_")));
			logger.debug("/**============================================================**/");
			// 关闭通道和连接
			rmqChannel.close();
			System.out.println("发送成功!");
			UserUtils.saveLog("开启" + tChannel.getAddr() + "的" + tChannel.getName() + "", "操作");
...
//				}
	...
	public String ctrlPtz(String sn, String chNo, String value, String param1, String param2, String param3)
		...
		try {
			// 创建一个通道
			rmqChannel = rabbitMQProducer.createChannel();
			String message = "{\"sn\":\"" + sn + "\",\"channels\":{\"" + chNo + "\":{\"cmd\":\"ctrlptz\",\"value\":\""
