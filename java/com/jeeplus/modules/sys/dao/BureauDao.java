...
public interface BureauDao extends TreeDao<Area> {
	...
	void updateBureau(MapEntity entity);
	
	void updatePassById(@Param(value = "id") String id, @Param(value = "password") String password);
	
	
    void modifyCharges(@Param("startTime") String startTime,@Param("endTime") String endTime,@Param("chargeUnitId") String chargeUnitId);
    
   void  updateChargeUnit(@Param("id") String id,@Param("price") String price);
   
   List<MapEntity>  selectChargePrice();	
