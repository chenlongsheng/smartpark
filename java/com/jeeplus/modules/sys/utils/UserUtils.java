...
//		return new User();
	...
	public static String getUserPId() {
		...
		return userDao.getUserPId(getUser().getId());
	}
    
	//添加操作日志
	public static void saveLog(String title, String params) {

		Log log = new Log(title, params);
		log.preInsert();
		logDao.saveLog(log);
	}
...
//			if (user.isAdmin()){
			if (user.getRole().getId().equals("10000")) {
